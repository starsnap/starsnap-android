...
import com.photo.starsnap.main.utils.NavigationRoute.AUTH_SIGNUP_USERNAME_ROUTE
...
fun SignupScreen(
    ...
    NavHost(navController = navController, startDestination = "SIGNUP") {
        navigation(startDestination = AUTH_SIGNUP_USERNAME_ROUTE, route = SIGNUP_ROUTE) {
            // 닉네임 입력 화면
            composable(AUTH_SIGNUP_USERNAME_ROUTE) {
                UserNameSignupScreen(viewModel, navController, onNavigateToLogin)
            }
            // 비밀번호 입력 화면
            composable(AUTH_SIGNUP_PASSWORD_ROUTE) {
                PasswordSignupScreen(viewModel, navController)
            }
            // 이메일 입력 화면
            composable(AUTH_SIGNUP_EMAIL_ROUTE) {
                EmailSignupScreen(viewModel, navController)
            }
            // 이메일 인증 화면
            composable(AUTH_SIGNUP_VERIFY_ROUTE) {
                VerifySignupScreen(viewModel, navController)
            }
            // 약관동의 화면
            composable(AUTH_SIGNUP_CONSENT_ROUTE) {
                ConsentSignupScreen(viewModel, navController)
            }
            // 마지막 화면
            composable(AUTH_SIGNUP_LOADING_ROUTE) {
